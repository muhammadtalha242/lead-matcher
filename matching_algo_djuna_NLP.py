...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("dejuna_matching_log.log"), logging.StreamHandler(sys.stdout)]
)

# Define file paths (update these to your local file paths)
purchase_file_path = './data/buyer_dejuna.csv'
...
# Define thresholds
COMBINED_TEXT_THRESHOLD = 0.2
BRANCHEN_THRESHOLD = 0.2
...
try:
    ...
    matches_df.to_excel("./matches/dejuna_high_quality_matches.xlsx", index=False)
    logging.info("Matches exported successfully to 'dejuna_high_quality_matches.xlsx'")
except Exception as e:
