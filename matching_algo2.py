...
#                 str(buyer['Titel']) if pd.notna(buyer['Titel']) else '',
#                 str(buyer['summary']) if pd.notna(buyer['summary']) else '',
#                 str(buyer['long description']) if pd.notna(buyer['long description']) else ''
#             ])).lower()
            
#             seller_text = ' '.join(filter(pd.notna, [
#                 str(seller['Titel']) if pd.notna(seller['Titel']) else '',
#                 str(seller['summary']) if pd.notna(seller['summary']) else '',
#                 str(seller['long description']) if pd.notna(seller['long description']) else ''
#             ])).lower()
...
#                     # Only create a match if both location and keywords match
#                     if has_keyword_match and has_location_match:
#                         match_info = {
#                             # Buyer information
...
#                             'buyer_title': buyer['Titel'],
#                             'buyer_long_description': buyer['long description'],
#                             'buyer_contact': buyer['contact details'],
                            
#                             # Seller information
#                             'seller_id': seller['source'].split('=')[-1] 
#                                 if pd.notna(seller['source']) else 'Unknown',
#                             'seller_location': seller['location (state + city)'],
#                             'seller_summary': seller['summary'],
#                             'seller_title': seller['Titel'],
#                             'seller_long_description': seller['long description'],
...
#             sellers_path='dejuna data feed - seller dejuna-new.csv',
                    ...
                    try:
                        ...
                        if has_keyword_match and has_location_match:
                            match_info = {
                                ...
                                'buyer_industry': seller.get('branche', ''),
